var LayoutConfiguration = {
		...
		if (menu.length) {
			...
			searchField.liveSearch({
						...
						show : function() {
							...
							portlet.show();
							portlet.parents('.lfr-content-category')
									.addClass('visible').removeClass('hidden')
									.show();
							portlet.parents('.lfr-add-content')
									.addClass('expanded')
			...
			searchField.liveSearch({
				...
				after : function() {
					if (!this.term) {
						instance.categories.addClass('hidden')
								.removeClass('visible').css('display', '');
						instance.categoryContainers.addClass('collapsed')
								.removeClass('expanded').css('display', '');
						instance.portlets.css('display', '');
					}

					if (this.term == "*") {
						instance.categories.addClass('visible')
								.removeClass('hidden');
						instance.categoryContainers.addClass('expanded')
				...
				exclude : function() {
					...
					var totalVisibleChildren = categoryContent
							.find('> div:visible').length;
	...
	toggle : function(ppid) {
		...
		if (!instance.menu) {
			...
			// }
			// });
			Ext.MessageBox.wait("正在加载数据", "加载中");
			jQuery.ajax({
						url : url,
						data : {
							p_l_id : plid,
						...
						success : function(message) {
							...
							PortletsHandler.displayWin(message);
							Ext.MessageBox.hide();
	...
	customAddPortlet : function(portletMetaData, options) {
		var instance = this;
	...
	_loadContent : function() {
		...
		var warp = jQuery("#ucit_njm_custom_portletContent");
		instance._portletItems = warp.find('div.lfr-portlet-item');
		var portlets = instance._portletItems;

		// portlets.find('a').click(function(event) {
		...
		instance._helper = jQuery(Liferay.Template.PORTLET)
				.css('z-index', 9999);
		...
		if (Liferay.Layout.isFreeForm) {
			...
			if (!instance._sortColumns || !instance._sortableInstance) {
				...
				instance._sortableInstance = instance._sortColumns
						.data('sortable');
			}
			...
			if (!instance._eventsBound) {
				sortColumns.bind('sortreceive.sortable', function(event, ui) {
							if (ui.item.is('.lfr-portlet-item')
									&& ui.sender.is('.lfr-portlet-item')
									&& !sortableInstance.dragging) {
				...
				sortColumns.bind('sortactivate.sortable', function(event) {
							Liferay.Layout.Columns.startDragging();
							sortableInstance.refreshPositions(true);
						});

				sortColumns.bind('sortstart.sortable', function(event, ui) {
							if (ui.item.is('.lfr-portlet-item')) {
								ui.placeholder.css({
											height : 200,
											width : 300
		...
		jQuery('.lfr-add-content > h2').click(function() {
	...
	_loadPortletFiles : function(portletMetaData) {
		...
		jQuery.each(headerCSS, function(i, n) {
					head.prepend('<link href="' + this
		...
		if (Liferay.Browser.isIe()) {
			...
			jQuery('link.lfr-css-file').each(function(i) {
						document.createStyleSheet(this.href);
					});
		}

		jQuery.each(footerCSS, function(i, n) {
					docBody.append('<link href="' + this
							+ '" rel="stylesheet" type="text/css" />');
				});
	},
	...
	_onPortletClose : function(event, portletData) {
		// PortletsHandler.onPortletClose(event, portletData);
		var instance = this;
		// var popup = jQuery('#portal_add_content');
		var addBtnId = "portletAddBtn_" + portletData.portletId;
		var addBtn = document.getElementById(addBtnId);
		if (addBtn) {
			addBtn.setAttribute("isUsed", "false");
		}
		var popup = jQuery('#ucit_njm_custom_portletContent');
		var item = popup.find('.lfr-portlet-item[plid=' + portletData.plid
				+ '][portletId=' + portletData.portletId
				+ '][instanceable=false]');
